...
- **Data residency.** Cloudflare at the edge, Cloudflare R2 in Western Europe for recorded audio, Hetzner in Germany (Falkenstein and Nürnberg) for live speech, Nebius for free-tier inference: the primary model in the United Kingdom under the EU adequacy decision, the fallback in Finland. The worker calls each model through the API host of its own region and checks the region the provider publishes for the model before each request, re-read hourly. A model published elsewhere is not asked. A daily probe reads the same metadata and alerts the operator channel on any drift.
