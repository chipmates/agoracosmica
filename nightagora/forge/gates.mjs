...
try {
  ...
  for (const tier of TIERS) {
    ...
    say(`  ${tier.padEnd(9)} ${r.cost.draws} draws  ${r.cost.triangles} tris  p50 ${r.cost.frameMsP50} ms  p95 ${r.cost.frameMsP95} ms`)
  }
  ...
  say(`  ${'calm-phone'.padEnd(9)} ${phone.cost.draws} draws  ${phone.cost.triangles} tris  p50 ${phone.cost.frameMsP50} ms  p95 ${phone.cost.frameMsP95} ms`)
...
for (const [name, r] of Object.entries(tiers)) {
  ...
  if (r.cost.textureMB > b.textureMB) overBudget.push(`${name}: ${r.cost.textureMB} MB over ${b.textureMB}`)
}
