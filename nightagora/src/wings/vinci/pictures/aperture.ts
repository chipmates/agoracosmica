...
type N = any
const { clamp, dot, float, max, mix, normalGeometry, positionWorld, pow, vec3 } =
...
export function apertureTone(light: ApertureLight, normal: N = normalGeometry): { tone: N; far: N } {
  ...
  const lit = max(raw, float(light.reference * light.floorFraction))
  const tone = clamp(pow(lit.div(light.reference), light.compression), light.range[0], light.range[1])
...
export function plateUnderAperture(light: ApertureLight): N {
  ...
  const fall = clamp(tone.div(light.range[1]), light.range[0] / light.range[1], 1)
