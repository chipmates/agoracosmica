...
console.log(JSON.stringify({ checker: 'vinci-collection-supper-room', ok: failures.length === 0,
  scope: 'The supper room as the runtime builds it: the end wall\'s plaster and reveal, the wall over the coping, the south wall, the bay\'s and the nave\'s roofs, the step, the top light\'s well, diffuser and glass, the north lintel and wall, the east wall and its door, the fins, the floor and the sill\'s dress, and the outside\'s parapets, lantern curb, bronze copings, lid frame and spouts, decks, base course and splash stone; every triangle of each. Every certificate route, approach, leg and wall span at its saved near and gait envelope plus a margin, every recorded corner ball, every station eye and viewing eye at its own near radius; the parachute\'s cloth and the crane\'s swept bound kept outside the room. The room stands outside the rail construction fingerprint.',
  marginM: MARGIN_M,
