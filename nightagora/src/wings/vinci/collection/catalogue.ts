 ...
 * certainty class is the label's own word, shown only where the attribution
 * is not a plain one. The word that says the wall shows a reproduction is
 * read from the card data and left out while the card data holds none.
 */
import cardsRaw from '../data/cards.json?raw'
import { policyCertainty } from '../pictures/policy-label'
...
export function hangCatalogue(work: PictureWork, face: 'front' | 'reverse' | null | undefined,
  ...
  const kind = QUALIFIED.has(work.attribution_certainty) ? policyCertainty(work, entries.length > 0, entries).word[language] : null
