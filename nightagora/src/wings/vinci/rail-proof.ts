...
export function createRailGeometryAuthority(roots: readonly Object3D[]) {
  ...
  const ready = (canHash ? railGeometryFingerprint(roots) : Promise.resolve('')).then(async hash => {
    ...
    // A proof that held is news, not a warning, so a page says it on the info
    // channel; in Node it stays on stderr, because the offline checkers run
    // this module there and a line on stdout is a line inside their JSON.
    const say = typeof document === 'undefined' ? console.warn : console.info
    if (exact) say('rail proof: the exact geometry hash')
    else if (canHash) say('rail proof: the certified signature, inside its tolerance')
    else say('rail proof: the certified signature, inside its tolerance. This page is not secure, so it has no way to hash the geometry (that needs https or localhost). The strict hash on a secure page is unchanged.')
  }).catch(error => {
