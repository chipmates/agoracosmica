...
The current texture policy and serial loading order are implemented in `loadMachineMaterial` in [parts.ts](parts.ts). Machine requests share a serial queue per stack, and each stands back for up to 30 seconds while the page's own library loads are in flight. That wait is a courtesy to their decodes, never a condition: a slow page delays a machine and never drops it. New linen/forged-iron sets keep full-tier maps; other new sets use 1024-pixel albedo plus procedural detail. The narrowed budget is held only across the request itself, because the library takes a set's budget at the moment it is asked for, so no other caller's set can be asked for while it stands. Cached host sets retain their maps. Use a fresh route/stack for a tier change. A set the library cannot dress is logged as an error and handed over without its maps, so its parts are still built and the failure is read below.

Every machine built through `buildMachine` is read by `machinesStanding()`, also `window.__naMachines()` on the page: machines, mounted, dressed, outstanding, the slugs still waiting and the errors, with `complete` true only when every part of every machine is mounted and dressed. The wing adds `outstanding` to its pending count and the errors to its error list, so `window.__forge.state().texturesPending` stays above zero until every machine is whole and `textureErrors` names any that cannot be. The bench's `ready` and `window.__forge.machine().parts` carry the same reading for its one machine and its supports.

The bench accepts `/bench/vinci/<slug>` and the existing `/bench/vinci/machines/<slug>` addresses. `window.__forge.jump('bench', {slug})` mounts the same station. `window.__forge.machine()` reports its slug, period, time, joints, declared bounds, occupied bounds and readiness; it returns null outside a machine bench. The broader `window.__forge.bench()` interface remains available for other bench kinds. `?benchMetrics=1` is an explicit diagnostic view: after at least 121 submitted frames and 1 second it displays renderer cost, timing, memory and measured target sizes. It is supplemental evidence, not an official cost or trusted-input gate.
