// "Play intro" trailer button for the public figure page hero.
// Reuses the standalone useFigureTrailer hook (HTML5 audio, tap-to-play,
// webm with mp3 fallback, served from R2). Same ~50s trailer asset as the
// in-app figure carousel.
// Remove this file when stripping marketing pages from a fork

import { usePublicLang } from './PublicLangContext';
import { useFigureTrailer } from '../../hooks/useFigureTrailer';

interface PublicTrailerButtonProps {
  figureId: string;
}

export default function PublicTrailerButton({ figureId }: PublicTrailerButtonProps) {
  const { t, lang } = usePublicLang();
  const trailer = useFigureTrailer();
  const status = trailer.activeId === figureId ? trailer.status : 'idle';
  const engaged = status === 'loading' || status === 'playing';

  return (
    <button
      type="button"
      className={`pub-trailer ${engaged ? 'pub-trailer--on' : ''}`}
      onClick={() => trailer.toggle(figureId, lang)}
      aria-pressed={engaged}
      aria-label={engaged ? t('figures.pauseIntro') : t('figures.playIntro')}
    >
      <span className="pub-trailer__icon" aria-hidden="true">
        {engaged ? (
          <svg viewBox="0 0 16 16" width="13" height="13" fill="currentColor">
            <path d="M4 3h3v10H4zM9 3h3v10H9z" />
          </svg>
        ) : (
          <svg viewBox="0 0 16 16" width="13" height="13" fill="currentColor">
            <path d="M4 3l9 5-9 5z" />
          </svg>
        )}
      </span>
      <span className="pub-trailer__label">
        {engaged ? t('figures.pauseIntro') : t('figures.playIntro')}
      </span>
      {!engaged && <span className="pub-trailer__dur">0:50</span>}
    </button>
  );
}
