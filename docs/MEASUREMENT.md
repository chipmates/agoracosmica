...
| Source class | `ad_google`, `ad_reddit`, `search`, `assistant`, `edu`, `mail`, `messenger`, `code`, `news`, `wiki`, `directory`, `reddit`, `social`, `referral` or `direct`, a closed list of fifteen values, set only on a landing pageview. It is worked out in the browser from the host of the referrer (never the referrer string, never a path) and from whether the landing URL carried an ad parameter (never the parameter's value). Both are read once and thrown away, nothing is stored on the device to work it out, nothing is forwarded anywhere, no click id is ever part of it, and the class value is the only thing that leaves the browser. The server accepts nothing outside this list. Like country and device class it describes the request, not the person who made it. Changed on 2026-09-21: the seven classes from `edu` to `directory` were added, and Hacker News moved from `social` to `news`. It had sent no landings under `social` before that day, so both series stay readable across the change | Know whether people arrive from search, from an AI assistant, from a classroom, a newsletter, a chat app, a code host, a wiki, a software directory, Reddit, a link someone posted, or straight to the address bar. That is what decides where the project spends its hours |
...
The four `ask_listen_*` steps count the ask that sits under a paused chapter. `ask_listen_shown` fires when the invitation appears (once per chapter play, so a pause for a sip is not counted twice), `ask_listen_sent` when a question goes out, and `ask_listen_resumed` when the chapter is picked up again after the answer. `ask_listen_resume_failed` counts the case nobody reports: the chapter was asked to pick up and no sound followed within a few seconds, so the surface falls back to the paused state. Each row carries the figure id and `story` in the content slot, the same labels every other step here carries, and nothing about the question itself.
