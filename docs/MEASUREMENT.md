...
The four `ad_consent_*` steps count the consent question itself: how often it appeared, and how often the answer was yes, no, or ignored. They are plain totals, one per step per tab. The row holds the step name, the sanitized page path the question appeared on, the interface language, and the country and device class every row here carries. Nothing else. No click ID, no figure, and no key that ties a yes or a no back to a visit, the same as everywhere else on this page. The path and the language are in there because the question does not land the same way on a figure page as on the homepage, or in German as in English, and that is what we need to know to write it better. `ad_consent_shown` only counts once the card has been at least half in view for a full second, so the answers get compared against questions someone could actually see.
